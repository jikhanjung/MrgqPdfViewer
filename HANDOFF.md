...
> 2. **벡터 PDF 악보 분석 파이프라인** (`56aea3e`, 06-13): `data/` 디렉토리 탐색 작업, **앱 미통합**. 자동 넘김·메트로놈·스코어 팔로잉 장기 로드맵의 기반 (P02/P03).
>
> 그 전 오선(staff line) 렌더링 작업(P01 → P2-A/B/C)은 일단락됨(§1). P2-B 채택, 실기기(FHD)에서 dropout 해결 확인. QHD 모니터에서도 "그럭저럭 볼만한" 수준.

---

## 0. 방금 끝낸 작업 — 합주 Phase 0 & 악보 분석 (커밋 완료, 일부 미검증)

### 0-A. 합주 Phase 0 — 동기 페이지 넘김 (`ec26ac7`, ⚠️ 빌드·실측 미수행)

**무엇을**: 지휘자가 페이지 넘김 시 "실제 넘길 절대 시각(`turn_at = now + lead`)"을 함께
브로드캐스트 → 모든 기기(지휘자 포함)가 그 벽시계 시각에 동시에 넘김. 기존의
수신-즉시-넘김에서 오던 기기 간 시점 어긋남 완화. (설계: `devlog/20260613_P03_clock_sync_design.md` 접근법 A)

**변경 범위**:
- 메시지 프로토콜: `page_change` 에 `turn_at` 필드 추가. `setOnPageChangeReceived` /
  `broadcastPageChange` 시그니처를 turn_at 포함으로 확장 (Server/Client/Global/Viewer 일관 반영).
- `PdfViewerActivity`: 지휘자 예약 넘김(`conductorScheduledTurn`) + 연주자 예약 실행 +
  재브로드캐스트 억제 창(`suppressBroadcastUntil`) + onDestroy 정리.
- `SettingsActivity`: 협업 패널에 "동기 페이지 넘김" 토글 + "예약 시간" 설정.
  prefs `sync_page_turn_enabled`(기본 **OFF**) / `sync_turn_lead_ms`(기본 2000ms).
- 기본 OFF = 기존 즉시 넘김 동작 유지 (하위호환).

**검증 (다음 작업자 1순위)** — WSL 에서 빌드 불가, Windows Android Studio + **2기기** 필요:
1. `assembleDebug` → 두 기기 설치, 지휘자/연주자 연결.
2. 설정에서 "동기 페이지 넘김" ON. 지휘자가 넘길 때 두 기기가 동시에 넘어가는지 (lead 2000ms).
3. 토글 OFF 시 기존 즉시 넘김으로 동작하는지 (회귀).
4. lead 시간 조정 시 반영, onDestroy 후 예약 콜백 누수 없는지.
5. 벽시계 동기화 전제 — 두 기기 시각 차이가 lead 보다 크면 어긋남. NTP 의존성 확인 (P03 후속: clock sync 정밀화).

상세: `devlog/20260614_038_phase0_sync_page_turn_implementation.md`,
계획: `devlog/20260613_P02_score_sync_autoturn_plan.md`(전체 단계), `_P03_clock_sync_design.md`.

### 0-B. 벡터 PDF 악보 분석 파이프라인 (`56aea3e`, 앱 미통합 / 탐색 작업)

`data/` 디렉토리에 Python 탐색 스크립트 일체. **앱 코드와 무관**, 자동 넘김·합주 음원
기능의 사전 조사:
- `segment_score.py`: 콘텐츠 스트림 CTM 추적 파싱 → 26시스템/81마디 검출
  (마디 번호 Sibelius 인쇄본과 완전 일치), `Moldau0607_layout.json` 출력.
- `build_parts.py`: 연주자별 파트보 PDF 5종.
- `pitch_poc.py` / `rest_poc.py`: OMR (음고·쉼표 인식, 검증 오버레이).
- `dur_detect.py` / `transcribe.py` / `multi_synth2.py`: 전사 + 5파트 합주 MP3 합성.
- `*.wav` 는 재생성 가능 중간 산출물이라 `.gitignore`.

상세: `devlog/20260613_037_score_analysis_work.md`.

---

## 1. 그 전 작업 — P2 (오선 dropout, 커밋 완료)

> 아래 P2-A/B 의 "미커밋"·"검증 예정" 서술은 작성 당시 기준. **현재 P2-B 는 커밋 `47507d7` 로 적용·푸시 완료, P2-C 는 `2fce7ca` 에 revert 기록 완료.** 자세한 최종 결과는 §1.5 참조.
...
- **QHD 모니터**: 1.333× 업스케일 경유에도 실사용상 "그럭저럭 볼만한" 수준으로 확인됨. UHD 교체는 더 이상 시급하지 않음(§4-1 참조).
...
- **합주 Phase 0 실기기 검증** (§0-A): 빌드·2기기 검증 미수행. 가장 시급.
- **합주 자동 넘김 후속 단계** (P02): Phase 0(동기 넘김) 다음은 Phase 1(자동 넘김 + 메트로놈) → 스코어 팔로잉. `devlog/20260613_P02_score_sync_autoturn_plan.md`.
- **악보 분석 파이프라인 앱 통합** (§0-B): 현재 `data/` 탐색 단계. 앱에 음원/자동넘김으로 들이려면 별도 설계.
- **코드 cleanup** (§4-2): `calculateOptimalScale` 시그니처/dead field 정리. 저위험.
- **P4 — 사용자 환경 가이드** (§4-1): README/설정 도움말에 모니터 권장 사항 추가. 미착수.
- **P5 — PDFium/MuPDF** (§5): 잔존 두께 미세차까지 잡으려면. 장기·고비용.
- **v0.2.x 로드맵** (§4-3): 신규 기능.
