...
### 🟡 갭 1: 테스트 커버리지 (2026-08-15 착수 → 09-13 현재 단위 120개 · 계측 45개)
...
4. [x] ~~**Room 마이그레이션**~~ — 스키마 export 와 드리프트 게이트(`0087ca6`, `713af4e`)에 이어
   `MusicDatabaseMigrationTest`(계측 5개)를 붙였다 (2026-09-13). v1/v2/v3 → v4 를 4.json 과
   대조하고, v2→v3 픽셀→퍼센티지 변환을 값으로 확인하고, 앱과 같은 마이그레이션 목록
   (`MusicDatabase.ALL_MIGRATIONS`)으로 v1 DB 를 Room 으로 열어 DAO 를 돌린다.
   - **정정**: 이전에 "과거 버전 스키마 JSON 이 갖춰졌다"고 적었으나 틀렸다. export 를 켠 시점이
     v4 라 `app/schemas` 에는 `4.json` 만 있고, `helper.createDatabase(name, 1)` 은 쓸 수 없다.
     옛 DB 는 git 히스토리(v1 `96c23d9`, v3 `dc2c09f`)의 엔티티로 복원한 SQL 로 직접 만든다.
     v2 는 커밋된 적이 없어 MIGRATION_1_2/2_3 에서 역산했다.
   - **발견**: `MIGRATION_3_4` 는 `user_preferences` 를 복사 없이 DROP 후 재생성한다. v1~v3 에서
     올라온 기기는 파일별 표시 설정이 전부 초기화됐다. 배포된 마이그레이션이라 고치지 않고
     테스트로 현재 동작을 고정했다.
   - 검증: 로컬 API 30 에뮬레이터에서 11/11 통과. `MIGRATION_3_4` 의 `ON DELETE CASCADE` 를
     일부러 지우면 4개가 실패하는 것까지 확인했다 (공회전 게이트가 아님).
...
4. [x] ~~**Room 마이그레이션 테스트**~~ (2026-09-13, `MusicDatabaseMigrationTest`). 다음에
   version 을 올릴 때는 `LATEST_VERSION` 과 옛 스키마 SQL 을 함께 갱신할 것 — 이제는 export 가
   켜져 있으므로 v4 이후 버전은 `helper.createDatabase` 로 만들 수 있다
5. **계측 테스트를 release 빌드로도** — 지금은 `connectedDebugAndroidTest` 라, R8 난독화가
   Room 의 enum 저장을 깨는 회귀는 "앱이 뜬다"까지만 확인된다.
   **PdfBox(문서 정보·악보 분석)도 같은 처지다** — release 스모크의 문서 정보 검사는 셸이 넣은 샘플을 앱이
   못 봐서 API 21/30/34 모두 건너뛴다 (devlog #047 §5). 테스트가 앱 프로세스 안에서 샘플을 쓰면 이 문제가 사라진다.
