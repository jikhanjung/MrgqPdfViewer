...
| CI | 🟡 2026-08-15 단위 테스트 14개 + Lint 게이트 도입. **여전히 커버리지는 렌더 튜닝 로직뿐** |
...
**아직 커버되지 않은, 실제로 고장났던 영역:**

1. **렌더 좌표 계산** — fitScale·클리핑·두 페이지 배치. #042 에서 정수 좌표 버그를 찾은 바로
   그 코드인데, **같은 공식이 세 곳에 복제**돼 있다
   (`PageCache.renderPageToTargetBitmap`, `renderPageAtSinglePageTarget`,
   `renderPageAtTwoPageTarget`). 가이드 `code-quality.md` §7 이 경고하는 "삼중 구현"이고,
   드리프트하면 캐시 히트/미스가 다르게 렌더된다(P01 이 실제로 겪은 증상).
   → **순수 함수로 추출 + characterization 테스트**가 다음 우선순위. 가이드 §4 의
   "커버되지 않은 코드를 리팩터링하기 **전에** 골든 테스트를 쓴다"에 정확히 해당한다.
2. **파일 목록 정렬·인덱스** — #029(10+ 파일 정렬), #030~#032(클릭한 파일과 다른 파일이
   열리던 race condition). 순수 로직으로 뽑을 수 있다.
3. **합주 메시지 프로토콜** — JSON 직렬화/파싱은 기기 없이 테스트 가능.
   `turn_at` 누락(구버전) 하위호환 같은 계약이 지금 아무 데도 고정돼 있지 않다.
4. **Room 마이그레이션** — v2→v3 픽셀→퍼센티지 변환. Room 은 마이그레이션 테스트를 공식
   지원한다. 과거에 마이그레이션 오류로 DisplayMode 저장이 깨진 적이 있다.
5. **에뮬레이터 스모크 (API 21 / 30 / 34)** — 가이드 §1 OS 매트릭스의 대응물.
   `reactivecircus/android-emulator-runner`. **위 Base64/getColor 건이 정확히 이 매트릭스가
   잡았을 버그다.**
...
1. [x] ~~순수 로직 단위 테스트 + `./gradlew lint` 를 CI 게이트로~~ (2026-08-15 완료)
2. **렌더 좌표 계산을 순수 함수로 추출 + characterization 테스트** — 삼중 구현을 하나로
   합치면서 동작 보존을 증명한다. 이 프로젝트에서 가장 자주 깨진 코드다
3. **에뮬레이터 스모크 (API 21 / 30 / 34)** — minSdk 21 을 선언만 하고 검증한 적이 없다.
   Base64/getColor 건이 정확히 이 매트릭스가 잡았을 버그다
