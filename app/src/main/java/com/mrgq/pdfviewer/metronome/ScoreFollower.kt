...
class ScoreFollower(measures: List<ScoreMeasure>, startMeasureNumber: Int) {
    ...
    sealed interface Position {
        ...
        data class CountIn(val beatInBar: Int, val beatsPerBar: Int) : Position
    ...
    private val beatsPerMeasure: IntArray
    ...
    init {
        ...
        val beats = ordered.map { m -> (m.timeSigNumerator ?: carried).also { carried = it } }

        playing = ordered.subList(startIndex, ordered.size)
        beatsPerMeasure = IntArray(playing.size) { (beats[startIndex + it] ?: FALLBACK_BEATS).coerceIn(1, MetronomeClock.MAX_BEATS) }
        countInBeats = beatsPerMeasure.first()

        firstBeat = LongArray(playing.size)
        var beat = countInBeats.toLong()
        for (i in playing.indices) {
            firstBeat[i] = beat
            beat += beatsPerMeasure[i]
        }
        totalBeats = beat
    }

    fun positionAt(beatIndex: Long): Position {
        if (beatIndex < countInBeats) return Position.CountIn(beatIndex.coerceAtLeast(0).toInt(), countInBeats)
        ...
        return Position.InMeasure(
            ...
            beatsInMeasure = beatsPerMeasure[lo],
            next = playing.getOrNull(lo + 1),
        )
    }

    /** 강박 위치 — [MetronomeEngine.barPosition] 으로 넘긴다. */
    fun beatInBarAt(beatIndex: Long): Int = when (val p = positionAt(beatIndex)) {
        is Position.CountIn -> p.beatInBar
        is Position.InMeasure -> p.beatInMeasure
        Position.Finished -> 0
    }

    companion object {
        /** 연동할 수 없는 마디에 쓸 일은 없지만(시작 가능한 마디만 고르게 한다) 방어적으로 둔다. */
        private const val FALLBACK_BEATS = MetronomeClock.DEFAULT_BEATS
