...
class PdfViewerApplication : Application() {
    ...
    override fun onTerminate() {
        super.onTerminate()
        Log.d(TAG, "Application 종료 - 협업 모드 정리 중...")
        
        // 모든 협업 모드 강제 정리
        try {
            GlobalCollaborationManager.getInstance().deactivateCollaborationMode()
            Log.d(TAG, "협업 모드 정리 완료")
        } catch (e: Exception) {
            Log.e(TAG, "협업 모드 정리 중 오류", e)
        }
    }
    
    override fun onLowMemory() {
        super.onLowMemory()
        Log.w(TAG, "메모리 부족 - 협업 모드 정리")
        
        // 메모리 부족 시에도 협업 모드 정리
        try {
            GlobalCollaborationManager.getInstance().deactivateCollaborationMode()
        } catch (e: Exception) {
            Log.e(TAG, "메모리 부족 시 협업 모드 정리 중 오류", e)
        }
    }
    ...
    override fun onTrimMemory(level: Int) {
        ...
        when (level) {
            ...
            TRIM_MEMORY_BACKGROUND -> {
                Log.w(TAG, "메모리 트림 (레벨: $level) - 협업 모드 정리")
                try {
                    GlobalCollaborationManager.getInstance().deactivateCollaborationMode()
                } catch (e: Exception) {
                    Log.e(TAG, "메모리 트림 시 협업 모드 정리 중 오류", e)
                }
            }
        }
    }
