...
dump_crash() {
  adb logcat -d -b crash 2>/dev/null | tail -60 || adb logcat -d | tail -60
}
...
# 프로세스 생존 확인. API 21 의 toolbox 에는 pidof 가 없고 ps 에 -A 도 없다.
# API 26+ 의 toybox 는 그 반대라, 둘 다 시도한 결과를 합쳐서 본다.
ps_out="$(adb shell ps -A 2>/dev/null || true)
$(adb shell ps 2>/dev/null || true)"

if ! printf '%s' "$ps_out" | grep -q "$PKG"; then
  echo "::error::release APK 가 실행 직후 죽었습니다 (API $API_LEVEL)"
  dump_crash
  exit 1
fi
