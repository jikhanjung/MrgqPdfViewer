...
> 메트로놈을 **연주하면서 쓰기 편하게** — 빠른 6/8 은 점4분음표로 세고, 연주 중 ↑ 키로 멈췄다가 이어서 · 다른 마디부터 다시 시작할 수 있다.
>
> - DB 는 v10 → v11 로 자동 마이그레이션된다 (메트로놈 점음표 박 컬럼 추가, 기존 설정 보존).
> - 이번 두 기능(점음표 박, ↑ 메뉴 · 일시정지)은 **에뮬레이터에서만 확인했다** (리모컨 키 이벤트로 전 흐름). 실기기 확인 전.

### 🥁 겹박자를 점4분음표로 세기 (devlog #052)
- 6/8 · 9/8 · 12/8 같은 겹박자에서 **세는 단위**를 고른다: `8분음표 6박`(지금까지 방식, 강약약중약약) 또는
  `점4분음표 2박`(강 · 중). 곡 빠르기에 따라 달라서 파일마다 저장한다.
- **바꿔도 빠르기는 그대로** — BPM 이 환산된다 (8분음표 216 ↔ 점4분음표 72). 점4분음표 기준 템포를 그대로 넣으면 되고,
  빠른 6/8 이 240 BPM 상한에 막히던 문제가 풀렸다.
- 8분음표로 세는데 BPM 이 180 을 넘으면 점4분음표로 세기를 권하는 안내가 보인다.
- 악보 연동도 같은 단위로 센다 — 예비박 2박, 한 마디 2박.
- 화면 왼쪽 위 박 표시: `6/8 · ♩. 72` 에 점 2개.
- 템포 하한 30 → 20 (느린 겹박자를 점4분음표로 바꿔도 빠르기가 유지되게).

### ⏸️ 메트로놈 메뉴(↑)와 일시정지 (devlog #053)
- 악보 화면에서 **↑ 키로 메트로놈 메뉴**를 연다 — 시작 · 정지 · 설정을 PDF 표시 옵션을 거치지 않고 바로.
- **악보 연동 중에 메뉴를 띄우면 일시정지**한다 (↑ 메뉴, OK 길게 누르는 PDF 표시 옵션 모두). 메뉴를 닫을 때 고른다:
  - **이어서** — 멈춘 마디의 처음부터, 한 마디 예비박 뒤
  - **마디 골라 다시 시작** — 멈춘 마디에 커서가 놓인 채 시작 마디 선택으로
  - **정지** — 고르지 않고 뒤로 닫아도 정지
- 일시정지 중 "메트로놈 설정…" 에서 템포·박자를 바꾸고 닫으면 다시 고를 수 있다 — 바꾼 설정으로 이어서.
- 멈춘 마디는 파란 상자로 표시되고, 다음에 마디를 고를 때 커서가 마지막으로 멈춘 마디에서 시작한다.

---

## [0.2.1] - 2026-09-13

> 메트로놈이 **8분의 6박자 같은 박자를 제대로 센다.** 지금까지는 "마디당 N박"(사실상 x/4)만 고를 수 있었다.
>
> - DB 는 v9 → v10 으로 자동 마이그레이션된다 (메트로놈 박 단위 컬럼 추가, 기존 설정 보존).
> - 실기기(Z18TV Pro)에서 메트로놈 · 악보 연동 · 박자 선택 모두 확인했다.
> - 박은 박자표 아래 숫자의 음표다. 빠른 6/8(점4분음표 90 이상)은 8분음표 기준 240 BPM 상한에 막힌다.

### 🥁 메트로놈 박자 선택 (devlog #051)
- **박자를 분자/분모로 고른다.** 지금까지는 "마디당 N박"(사실상 x/4)뿐이라 6/8 곡을 제대로 셀 수 없었다.
  대화상자에 **흔한 박자 버튼**(4/4 · 3/4 · 2/4 · 6/8 · 2/2 · 12/8 · 3/8 · 9/8 · 5/4)과 **직접 고르기**
  (마디당 박 수 슬라이더 1~12 + 박 단위 /2 · /4 · /8 · /16).
- 박은 분모 음표이고 **BPM 도 그 음표 기준**이다 — 6/8 이면 "8분음표 = 180 BPM".
- **겹박자(분자 6·9·12)는 1박 강 · 4박 중간** (12/8 은 4·7·10박 중간). 중간 세기 클릭음을 새로 넣었고,
  박 표시에서 노란 점으로 켜지며 3박 묶음 사이가 벌어진다.
- 이 파일에서 박자를 고른 적이 없으면 **악보에서 읽은 박자표로 채운다** (몰다우 → 6/8). 분석 전인 파일은 이때 분석한다.
- 화면 왼쪽 위 박 표시에 박자표와 박 단위 음표를 함께 보인다 (예: `6/8 · ♪ 180`).
- 악보 연동 중에는 마디 길이와 강세 모두 그 마디의 박자표를 따른다 (박자가 바뀌는 마디 포함).

---

## [0.2.0] - 2026-09-13

> 악보를 **읽는** 첫 버전. PDF 에서 제목·작성자, 시스템·마디, 박자표를 읽고, 그 위에서 **메트로놈이 마디를 따라가며
> 페이지를 넘긴다** (P02 Phase 1).
>
> - ⚠️ **minSdk 21 → 30** — Android 11 이상에만 설치된다. 실사용 기기(Google TV Streamer, Z18TV Pro)는 둘 다 Android 14.
> - DB 는 v4 → v9 로 자동 마이그레이션된다 (파일별 표시 설정 보존, 실기기 v0.1.12 데이터로 확인).
> - targetSdk 가 30 이라 Android 14 기기에서 설치 시 "이전 버전의 Android 용" 경고가 뜬다 — 허용하고 설치하면 된다.
> - 실기기(Z18TV Pro) 확인: 문서 정보, 악보 분석(8쪽 556ms), 마디 박스, 두 버그 수정. **메트로놈과 악보 연동은 에뮬레이터에서만 확인했다.**
> - 악보 분석·박자표는 **Sibelius → Microsoft Print to PDF 형식**에 맞춰져 있다. 다른 형식은 마디를 못 찾거나 박자표를 못 읽고, 그때 메트로놈은 일반 모드로 돈다.

### 🥁 메트로놈 악보 연동
- **메트로놈을 시작하면 악보에서 시작 마디를 고른다** — 현재 화면 첫 마디에 커서가 뜨고 ←→ 마디, ↑↓ 줄, OK 시작,
  뒤로 취소. 커서가 화면 밖 마디로 가면 페이지가 넘어간다.
- **한 마디 예비박** 뒤 **현재 마디를 노랗게 표시**하며 박자표대로 마디를 따라간다(강박도 악보 박자표대로).
  박은 박자표 아래 숫자의 음표다 (6/8 이면 8분음표 6박).
- **자동 넘김** — 다음 마디가 다른 페이지면 이 마디가 끝나기 **2박 전**에 넘긴다. 악보 끝에서 멈춘다.
- 연주 중 뒤로 키는 뷰어를 나가지 않고 메트로놈만 멈춘다.
- 박자표를 못 읽었거나 악보 분석이 안 되는 파일은 연동 없이 일반 메트로놈으로 돈다.
- 화면 왼쪽 위 박 표시를 반투명으로 — 박이 많은 박자(6/8)에서 악보 제목을 가리던 것을 완화.

### 🎼 박자표 읽기
- **악보 분석이 박자표(예: 6/8)를 읽어 마디마다 기록한다** — 보표 안에 x 가 같게 위아래로 놓인 숫자 쌍
  (분모는 2의 거듭제곱, 시스템 보표 과반에서 반복)을 찾는다. 박자표가 놓인 마디부터 다음 박자표 전까지 적용.
  Moldau 81 마디 전부 6/8 (골든 테스트). 숫자가 아닌 박자표(C 기호)는 못 읽는다.
- 경량 해석기가 텍스트 위치를 모으고, 글자 해독만 PdfBox 글꼴로 한다. 분석 시간 190ms → 300ms (에뮬레이터 13쪽).
- **Room 스키마 v9** — `score_measures` 에 박자 컬럼. 이미 분석된 파일은 다음에 다시 분석한다.

### 🎨 화면
- **PDF 표시 옵션에서 "선 선명도 (배율 · 감마)" 제거** — 측정으로 정한 기본값(oversample 1×, 감마 자동)만 쓴다.
  예전에 이 메뉴로 저장한 값은 되돌릴 방법이 없어지므로 앱 시작 시 지운다.
- 스플래시 저작권 표시 `© 2025-2026 MRGQ`

### ⚡ 악보 분석 속도·메모리
- **분석 경로에서 PdfBox 엔진을 빼고 경량 콘텐츠 해석기로 교체** — Moldau 13쪽 기준 할당 905MB → 19MB,
  7.4초 → 0.19초, GC 60여 회 → 4회 (에뮬레이터). PdfBox-Android 는 토큰화에만 633MB 를 할당하고 있었다.
  PdfBox 는 스트림 압축 해제·Form XObject 조회에만 쓴다.
- **Z18TV Pro 실기기**(Android 14, 32bit ARM, RAM 2GB)에서 8쪽 악보 첫 분석 556ms.
- 해석기가 기존 PdfBox 엔진과 같은 경로 박스를 내는지 전 페이지 대조 테스트로 고정.
- 상세: `devlog/20260913_049_score_analysis_memory.md`

### 🐛 실기기에서 발견해 고친 것
- **끝세로줄(가는 선 + 굵은 선)에 폭 5pt 짜리 가짜 마디가 생기던 문제** — 남은 폭이 최소 마디 폭(15pt) 이상일 때만
  오선 끝을 경계로 추가한다.
- **한글 작성자·제목이 `ì€—ìŸ‹ìŽ—` 처럼 깨지던 문제** — Microsoft Print to PDF 가 규격을 어기고 BOM 없는 UTF-8 을
  썼다. 원본 바이트가 엄격한 UTF-8 이면 UTF-8 로 읽는다. 이미 저장된 값은 **DB v8 마이그레이션**이 한 번 다시 읽게 한다.

### 📱 지원 범위
- **minSdk 21 → 30 (Android 11)** — 실사용 기기(Google TV Streamer, Z18TV Pro)는 둘 다 Android 14 이고, Android 11 을
  보수적 하한으로 둔다. (처음엔 Z18TV Pro 를 출고 사양대로 Android 11 로 알았으나 실기기는 Android 14 였다.)
  Android 5.0~10 기기에는 설치되지 않는다.
- CI 계측 매트릭스를 API 30/34 로 줄였다. 계기: API 21 에뮬레이터에서 악보 분석 골든 테스트가 메모리 부족으로
  죽어 42개 중 35개만 돌았다 (직전 커밋에서는 통과 — 한계에 걸쳐 있던 것).
- 상세: `devlog/20260913_048_min_sdk_30.md`

### 🥁 메트로놈
- **PDF 표시 옵션 → 메트로놈** — 템포 30–240 BPM(±1/±10 버튼), 마디당 1–12박(첫 박 강조), 클릭음 켜기·음량.
  실행 중에는 화면 왼쪽 위에 템포와 박 점이 표시된다. 실행 중에 바꾸면 다음 박부터 적용된다.
- **흔들리지 않는 박** — 타이머로 효과음을 울리지 않고 AudioTrack 스트림에 클릭 파형을 샘플 단위로 이어 쓴다.
  화면 박 표시도 실제 재생 위치를 따른다. 소리 장치를 못 열면 박 표시만 한다.
- **템포·박자는 파일별로 저장** — Room 스키마 v7 (`user_preferences.metronomeBpm`, `metronomeBeatsPerBar`,
  `MIGRATION_6_7`). 미설정이면 120 BPM · 4박.
- 뷰어를 벗어나거나 다른 곡으로 넘어가면 멈춘다.
- 상세: `devlog/20260913_047_metronome.md`

### 🎼 악보 구조 분석 — 마디 박스 오버레이
- **벡터 악보 PDF 에서 시스템·마디를 찾는다** — `data/segment_score.py` 를 Kotlin(PdfBox 그래픽 엔진)으로
  옮겼다. 파이썬 결과(Moldau 총보 26 시스템 / 81 마디)와 마디선·보표 좌표까지 0.6pt 이내로 일치 (골든 테스트).
  지원 형식: Sibelius → Microsoft Print to PDF. 다른 형식은 "마디 없음" 으로 넘어간다.
- **PDF 표시 옵션 → "마디 박스 표시 (악보 분석)"** — 켜면 파일마다 처음 한 번 분석하고, 마디마다 번호 붙은
  박스를 악보 위에 그린다. 두 페이지 모드·클리핑·중앙 여백을 렌더러와 같은 공식으로 따라간다.
- **Room 스키마 v6** — `score_measures` 테이블 + `pdf_files.scoreAnalyzedAt` (`MIGRATION_5_6`, 기존 데이터 보존).
  파일이 바뀌면 다시 분석한다.
- 에뮬레이터에서 13쪽 분석에 약 9초 — 실기기 측정 전.
- 상세: `devlog/20260913_046_score_layout_in_app.md`

### 🧪 CI
- release 스모크의 문서 정보 검사가 adb 종료 코드 대신 **앱 로그**로 판정한다. API 21 은 넣기에 성공하고도
  실패 코드를 돌려줘 검사를 건너뛰었고, API 34 는 넣기에 성공했는데 판정할 근거가 없었다.
  - ⚠️ 그 결과 CI 세 매트릭스 모두 앱이 샘플을 보지 못해(`LOADED 0 PDF FILES`) 검사를 **건너뛴다** — CI 는
    초록불이지만 release APK 의 PdfBox 동작은 로컬 API 29 에서만 확인됐다 (devlog #047 §5).
...
> v0.1.13 이후 쌓인 **품질 게이트 정비** 버전. 새 기능은 없다.
> 사용자가 보는 변화는 **크래시 수정 6건**과 **파일 목록 정렬 변경** 둘이다.
> 특히 API 21~22(Android 5.0~5.1) 에서는 그동안 실행 중 죽던 경로가 있었고,
> Android 8 미만에서는 합주 기능이 애초에 동작할 수 없었다.

### 🧪 테스트 · CI (테스트 0개 상태 해소)
- **단위 테스트 도입 + CI 게이트** — `testDebugUnitTest` 와 `lintDebug` 를 빌드 게이트로.
  테스트가 실제로 몇 개 돌았는지 세어 0 이면 실패시키는 안전장치 포함(테스트 소스가 통째로
  사라져도 gradle 은 성공하기 때문). 현재 단위 테스트 57개.
  - `RenderTuningTest`(14) — 측정으로 결정한 배율·감마 값을 코드로 고정
  - `PageGeometryTest`(16) — 렌더 좌표 계산
  - `PdfFileSorterTest`(14) — 파일 목록 정렬
  - `CollaborationProtocolTest`(13) — 합주 와이어 포맷, `turn_at` 누락 하위호환 포함
- **에뮬레이터 계측 테스트 + release APK 스모크** (API 21 / 30 / 34) — Room 저장 왕복을
  실제 기기에서 검증하고, minify 된 release APK 가 기동하는지 확인한다.
  - ⚠️ 도입(2026-08-15) 이후 3주간 세 매트릭스 전부 빨간불이었고, 2026-09-06 에야 통과했다.
    실패 네 겹 중 셋이 CI 스크립트 자체 문제였다 (서명 키 누락, dash 비호환, 러너의 줄 단위
    실행 모델). 상세: `devlog/20260906_043_instrumentation_ci_repair.md`
  - 이로써 **minSdk 21 이 처음으로 런타임 검증**됐고, **release APK 가 처음으로 실행**됐다
- **Room 스키마 export + 드리프트 게이트** — `app/schemas` 에 JSON 을 커밋하고, 엔티티를
  고쳤는데 version 을 안 올렸거나 스키마 커밋을 빠뜨리면 CI 가 막는다. 마이그레이션 테스트의
  전제이기도 하다.

### 🐛 버그 수정 (Lint 게이트 첫 실행에서 발견)
- **`java.util.Base64`(API 26+) 를 WebSocket 핸드셰이크에서 사용** — Android 8 미만에서 합주
  기능이 애초에 동작할 수 없었다. minSdk 21 을 선언만 하고 검증한 적이 없었다는 뜻이다.
- **`Context.getColor`(API 23+) ×4** — API 21~22 크래시
- 23자 초과 로그 태그 ×32 (API<24 예외), Leanback 런처인데 `android:banner` 누락
- `effectiveOversampleFactor` 의 비정상 크기 가드가 음수를 통과시키던 문제

### ♻️ 리팩터링
- **렌더 좌표 계산을 `PageGeometry` 로 추출** — 같은 공식이 세 곳에 복제돼 있었고, 드리프트하면
  캐시 히트와 미스가 다르게 렌더된다(P01 이 실제로 겪은 증상).
- **합주 와이어 포맷을 `CollaborationProtocol` 로 단일화** + dead code 제거
- **파일 목록 정렬을 자연 정렬 + 전순서로** — 클릭한 파일과 다른 파일이 열리던 과거 문제
  (#030~#032)의 구조적 원인이 정렬이 전순서가 아니었던 것이다. 10개 이상일 때 사용자가 보는
  순서가 바뀐다.
- 저장소 줄바꿈을 LF 로 통일 (CRLF/LF 혼재 해소)
...
> ✅ **렌더 품질은 실기기 확인 완료** (Google TV Streamer + 23.8" 4K 모니터, 2026-08-15):
> 1920×1080 네이티브 렌더 결과가 매우 좋고 사용상 문제 없음.
> ⚠️ 나머지 스모크(리모컨 롱프레스·웹서버·합주 회귀)는 미실시. 합주 Phase 0 동기 넘김은
> 실기기 미검증 상태(기본 OFF)로 포함됨.
...
- **기본값을 화면 세로 해상도에 연동**(`InkGamma.defaultFor`): 보정이 필요한 이유가 오선이 표시 픽셀 기준 1px 미만이기 때문이므로, 화면이 고와질수록 필요량이 준다. 실측 앵커(1080p → 2.0)와 중립점(2160p → 1.0) 사이를 선형 보간하고 그 아래는 외삽 — 720p 2.33 / 1080p 2.00 / QHD 1.67 / 4K 1.00. 사용자가 슬라이더로 저장한 값이 있으면 언제나 그쪽이 우선.

### ✨ oversample 제거 — 네이티브 렌더로 복귀 (선명도 대폭 개선)
- **`OVERSAMPLE_FACTOR` 기본값 4.0× → 1.0×**. PDFium 은 1픽셀 미만 stroke 를 device pixel 에 스냅하는데(데스크톱 뷰어가 얇은 선을 또렷하게 그리는 동작), oversample 을 걸면 그 1px 이 표시 기준 1/N px 로 희석되어 회색이 된다. **oversample 을 올릴수록 얇은 선은 흐려진다.**
  - 배율 스윕(A4 악보, 764×1080, 오선 50개): **1× → darkness 평균 1.000 / 최소 1.000 / 편차 0.000**. 2.5×(v0.1.11) 0.635, 4×(v0.1.12) 0.674, 4×+감마2.0 0.878. 두께 편차도 1× 가 가장 낮다.
  - 전제 정정: oversample 은 "2.5K/4K 화면에서 해상도를 최대한 활용"하려던 것이었으나, 앱 UI 가 1080p 로 고정임이 확인되어(#040) 전제가 소멸.
- **감마를 oversample 에 종속**: `InkGamma.defaultFor(screenHeight, oversample)`. 1× 에서는 오선이 이미 순수 검정이라 감마 보정이 불필요하므로 자동으로 1.0(꺼짐).
- **정수 좌표 보정** (1× 선명도 유지의 전제): `combineTwoPagesUnified` 의 비트맵 배치 좌표를 정수로, `setImageViewMatrix` 의 translate 를 정수로 반올림하고 배율이 1.0 에 근접하면 정확히 1.0 으로 스냅. 소수점 blit/scale 은 재샘플링을 유발해 device-pixel 스냅을 무효화한다.
- **설정 UI 확장**: "오선 진하기 (감마)" → **"선 선명도 (배율 · 감마)"**. 슬라이더 2개 + 빠른 버튼(`권장 1×` / `이전 4×+γ2`) + 실시간 미리보기. 전역 설정(`oversample_factor`, `ink_gamma`).
- **부수 효과**: 렌더 픽셀 수 13.2MP → 0.83MP (약 16배 감소), transient oversample 비트맵(~133MB) 소멸 → 렌더 속도·메모리 개선 예상.
- 측정 스크립트: `data/oversample_sweep.py`
