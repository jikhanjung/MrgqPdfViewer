...
- **언어**: Kotlin
- **아키텍처**: PdfViewerActivity + PageCache 중심 (v0.1.8 의 Manager 분리 시도는 미통합 상태로 남아 있다가 v0.1.10 에서 제거됨)
...
- [x] **PdfViewerActivity 아키텍처 리팩토링 시도 (부분 완료)**: 매니저 클래스들을 작성했으나 실제 호출부 통합은 미완료. v0.1.10 오선 렌더링 작업 중 dead code 로 확인되어 PdfPageManager 는 제거됨. 라이브 렌더는 PdfViewerActivity + PageCache 가 담당.
...
- [x] PDF 파일 전체 삭제 (앱 내)
- [x] PDF 렌더링 기본 기능
- [x] 고해상도 PDF 렌더링 (단일 단계 Matrix 렌더 + 2.5× oversample, v0.1.10+)
...
3. **고해상도 PDF 렌더링**
   - 단일 단계 Matrix 렌더 + 2.5× oversample (v0.1.10+)
