...
**현재 버전**: v0.1.5 (2025-07-06)  
**빌드 상태**: 🟢 빌드 가능  
**테스트 상태**: 🟢 기본 기능 테스트 완료
**최근 업데이트**: 포트 바인딩 문제 해결, 전역 파일 변경 처리, TV 스타일 설정 화면
...
### 핵심 파일
- `MainActivity.kt`: 메인 화면 및 파일 목록, 협업 파일 변경 처리
...
- `GlobalCollaborationManager.kt`: 전역 협업 상태 관리
...
5. **앱 종료 시 협업 모드 완전 정리 (강화)**
   - PdfViewerApplication 클래스로 강제 종료 상황 대응
   - MainActivity onDestroy에서 협업 모드 강제 해제
...
이번 업데이트로 **지휘자가 페이지를 넘길 때 연주자 페이지가 즉시 전환**되어 실시간 협업 성능이 크게 향상되었습니다.

## v0.1.5+ 추가 업데이트 내용 (2025-07-06)
...
2. **전역 파일 변경 처리 시스템**
   - MainActivity에 협업 콜백 추가 (setupCollaborationCallbacks)
   - SettingsActivity에 협업 콜백 추가
...
4. **TV 스타일 설정 화면 (SettingsActivityNew)**
   ...
   - 5개 주요 카테고리: 📂파일관리, 🌐웹서버, 🎼협업모드, 🔧시스템, 📊정보
