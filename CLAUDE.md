...
**CI 게이트**: 🟢 단위 테스트 156개 + Android Lint + 에뮬레이터 계측 48개·release APK 스모크(API 30/34 — ⚠️ 문서 정보 검사는 모든 매트릭스에서 건너뜀, #047 §5). 계측은 main 푸시와 수동 실행에서만 (회당 5~6분)  
...
- **데이터베이스**: Room 2.6.1 (SQLite 기반, 현재 v10 스키마 — `score_measures`(박자표 포함), 파일별 메트로놈(템포·박자 분자/분모) 포함. ⚠️ `pdf_files` 갱신에 `insertPdfFile`(REPLACE) 금지 — CASCADE 로 표시 설정 삭제)
