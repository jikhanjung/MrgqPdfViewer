...
**CI 게이트**: 🟢 단위 테스트 66개 + Android Lint + 에뮬레이터 계측 25개·release APK 스모크(API 21/30/34). 계측은 main 푸시와 수동 실행에서만 (회당 5~6분)  
...
- **데이터베이스**: Room 2.6.1 (SQLite 기반, 현재 v5 스키마. ⚠️ `pdf_files` 갱신에 `insertPdfFile`(REPLACE) 금지 — CASCADE 로 표시 설정 삭제)
