...
**v0.1.5+** - Android TV용 PDF 악보 리더 앱 (협업 모드 & TV UI 특화)
...
### 🎯 포트 바인딩 문제 해결
- **1초 소켓 타임아웃** 적용으로 앱 강제 종료 후 즉시 지휘자 모드 재활성화
- **Accept 스레드 반응형 종료**로 포트 충돌 완전 방지

### 🌐 전역 파일 변경 처리 
- **모든 화면에서 동기화**: 연주자가 파일 목록/설정 화면에 있어도 즉시 반응
- **자동 파일 다운로드**: 없는 파일은 진행률과 함께 자동 다운로드
- **화면 간 자동 전환**: 설정 화면 → 메인 화면 자동 이동

### 📱 TV 스타일 설정 화면
- **카테고리별 메뉴**: 5개 주요 카테고리로 단순화
- **리모컨 최적화**: 72dp 최소 터치 영역, DPAD 완벽 지원
- **직관적 아이콘**: 이모지 아이콘으로 기능 즉시 식별
- **단계별 설정**: 복잡한 스크롤 → 상세 패널 방식
