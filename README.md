...
**v0.1.6** - Android TV용 PDF 악보 리더 앱 (협업 모드 & TV UI 특화)
...
- **연결 성공률 100% 달성**: 콜백 생명주기 타이밍 이슈 근본 해결
- **일관된 모드 활성화**: 모드 활성화 → 콜백 설정 → 운영 시작 순서 보장
- **강화된 콜백 관리**: 🎯 prefix 로깅으로 전체 콜백 체인 추적 가능
- **UDP 포트 관리 완전 개선**: 포트 충돌 0%, 소켓 강제 종료 + 100ms 대기

### 🎨 TV 사용자 경험 혁신
- **TV 스타일 설정 화면**: 카테고리별 메뉴 구조 (📂파일관리, 🌐웹서버, 🎼협업모드, 🔧시스템, 📊정보)
- **리모컨 최적화**: 72dp 최소 터치 영역, 직관적 DPAD 탐색
- **설정 UI 컴포넌트**: SettingsItem 데이터 모델, 상세 패널 방식

### 🤝 협업 모드 안정성 대폭 개선
- **포트 바인딩 문제 완전 해결**: 1초 소켓 타임아웃, Accept 스레드 반응성
- **전역 파일 변경 처리**: 모든 화면에서 협업 반응, 자동 화면 전환
- **파일 다운로드 통합**: 중복 구현 제거, 일관된 다운로드 경험
...
### 📚 문서화
- **ENSEMBLE_MODE_RESTRUCTURING.md**: 재구조화 전 과정 기술 문서 완성
